...
## 渋谷エリア実装の必須チェック（#125/#126 の教訓）

### 独立 sim は「全 AABB」を検証すること

新エリア（坂・通路）の歩行可能性を独立 sim で検証する際、そのエリア固有の AABB だけを見てはいけない。
`SHIBUYA_AABBS` に累積されている**既存の全 AABB を含めて**検証する。

理由: 公園通り (#125) で独立 sim が公園通り固有 AABB のみを検証し、既存 AABB（大型ビジョンビル・
西フランキングスラブ・駅 AABB）との干渉を見落とした。sim は「OK」を返したが実機で通路が完全封鎖され、
入れない・素通り壁だらけ・隙間だらけになった（#126 で 5 件修正）。

### 新エリアの回廊が既存 AABB と重ならないか必ず確認

新しい通路/坂を追加する前に、その座標帯に既存の建物 AABB がないかチェックする。重なる場合は:

- **既存 AABB を分割して回廊部分を開口する**（駅 AABB の東西分割が例）
- **既存 AABB を通路外に縮小する**（ビジョンビルを東半分 `x[12,17]` のみにした例）

### AABB は回廊に 1u も食い込ませない

当たり判定を残す場合も、歩行帯（回廊）に食い込むと「見えない壁」になる。
メッシュ footprint ぴったりにし、**回廊側には最低 2u の余裕**を持たせる。

回廊東端 (local x) の計算例（公園通り南端 z=-32）:
> KDR centerline x ≈ -0.95 → corridor east edge = -0.95 + KDR_HW(11) = **+10.05**
> → AABB west edge は ≥12 にする（2u clearance）

### マージ前に必ず実ブラウザで歩く

独立 sim が OK でも実機で破綻しうる（上記の通り）。
新エリアは本番マージ前に実ブラウザで以下を人間が目視確認する:

- そのエリアに入れるか
- 端から端まで歩けるか
- 素通り壁・見えない壁・落下がないか

WebGL はヘッドレス Chrome では動かないため、必ず実ブラウザで確認すること。
