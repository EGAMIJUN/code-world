...
私はVB.NETでの業務システム開発経験はあるものの、React・Three.js・ゲーム開発はほぼ未経験です。実装の大部分はClaude Codeに任せ、自分は要件定義・レビュー・プレイテストに集中するスタイルで開発しました。

この記事では、ゲームの内容、技術スタック、実装した機能、開発中にハマったポイントを紹介します。

:::note info
本記事は実体験をもとに執筆しています。コードの大部分はClaude Codeによる生成ですが、動作検証・プレイテスト・記事執筆は人間が行っています。
:::
...
| 難易度ティア | grunt / miniboss / boss の3段階（HP 60/150/400） |
| 縦方向マップ | 侵入可能な建物8棟、ハシゴで屋上に登れる |
| 物理 | 重力、落下ダメージ |
| ウェーブシステム | Wave進行で敵が増加、全Wave制圧でミッションクリア |
| マルチプレイヤー | Socket.ioによるリアルタイム位置同期 |
...
| バックエンド | Hono / Better Auth / Socket.io |
| ジョブキュー | BullMQ |
| コード実行 | Judge0 + Docker |
...
ゲーム本体は `ThreeWorld.tsx` という約2,600行のコンポーネントに実装されています。
...
- 1体が射撃して足止めしている間に、他の敵が位置取りを変えるグループ連携
...
- レイキャスト頻度を毎フレームから3フレームに1回へ削減
- 街灯を24本から14本に削減、PointLightを全廃
...
Dependabotがdrizzle-ormのSQLインジェクション脆弱性を検知した際は、Claude Codeに対応を依頼しました。依存の更新、影響範囲の調査、テスト追加、CodeQLセキュリティスキャンの導入までを1つのPRで完了しています。
